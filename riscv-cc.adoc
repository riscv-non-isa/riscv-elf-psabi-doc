...
address. This ensures correct `va_arg` operation when arguments are spilled to
the stack.

The following table summarizes the register-pair ordering for a value that
occupies an aligned 2×XLEN register pair. Two orderings are distinguished:
_significance ordering_, in which the lower-numbered register holds the
low-order XLEN bits, and _memory-layout ordering_, in which the lower-numbered
register holds the XLEN bits at the lower memory address. On little-endian
these two orderings coincide; on big-endian they are opposite.

[cols="4,3,3"]
|===
| Value in an aligned 2×XLEN register pair | Named argument / return value | Variadic argument

| 2×XLEN scalar
| significance ordering
| memory-layout ordering

| `_BitInt(N)` with XLEN < N ≤ 2×XLEN (passed under the 2×XLEN-scalar rules)
| significance ordering
| memory-layout ordering

| Aggregate no larger than 2×XLEN bits
| memory-layout ordering
| memory-layout ordering
|===

Variadic values always use memory-layout ordering so that register-passed and
stack-passed variadic arguments share the same in-memory layout, which is
required for correct `va_arg` operation.

NOTE: On big-endian systems, only scalar-class values (2×XLEN scalars and
`_BitInt` passed under the scalar rules) change ordering between the named and
variadic paths. Aggregates use memory-layout ordering in both cases.
