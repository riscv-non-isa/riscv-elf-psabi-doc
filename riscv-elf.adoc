...
All data types in a program follow the byte ordering selected for that program.
The psABI assumes this data byte ordering is fixed for the duration of the
program; mechanisms for changing data endianness are system-level issues outside
the scope of this ABI.
