...
.2+| 66-190  .2+| *Reserved*                          .2+| -       |                   .2+| Reserved for future standard use
                                            <|
.2+| 191     .2+| RELOCID       .2+| Static  |                   .2+| Paired with a vendor-specific relocation, indicates which vendor owns the relocation.
                                            <|
...
`R_RISCV_RELOCID` relocation against the absolute section plus an addend
identifying the ID of the vendor.
...
be shown. Data contained in paired `RISCV_RELOCID` can be used to select the
...
Vendor identifiers are 32-bit values stored in the corresponding
`R_RISCV_RELOCID` relocation’s addend (irrespective of ELF class/XLEN) and must
be unique amongst all vendors providing custom relocations.

Since RISC-V toolchains already have a convention for prefixing instructions
with vendor identifiers, where these are fewer than 4 characters in length it
is suggested to use the ASCII values of these strings as a vendor identifier.
<<var-vendor-ids>> provides examples of such.
...
[%autowidth]
|===
| Vendor | ID (ascii) | ID (hex)

| Open Hardware Group   | cv | 0x6376
| SiFive                | sf | 0x7366
| T-Head                | th | 0x7468
| Ventana Micro Systems | vt | 0x7674
| Nuclei                | xl | 0x786C
|===
