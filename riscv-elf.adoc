...
.2+| 20      .2+| GOT_HI20      .2+| Static  | _U-Type_          .2+| High 20 bits of 32-bit PC-relative GOT access, `%got_pcrel_hi(symbol)`, the addend must be 0
                                            <| G + GOT - P
.2+| 21      .2+| TLS_GOT_HI20  .2+| Static  | _U-Type_          .2+| High 20 bits of 32-bit PC-relative TLS IE GOT access, macro `la.tls.ie`, the addend must be 0
                                            <|
.2+| 22      .2+| TLS_GD_HI20   .2+| Static  | _U-Type_          .2+| High 20 bits of 32-bit PC-relative TLS GD GOT reference, macro `la.tls.gd`, the addend must be 0
                                            <|
...
.2+| 61      .2+| SUB_ULEB128   .2+| Static  | _ULEB128_         .2+| Must be placed immediately after a SET_ULEB128 with the same offset. Local label subtraction <<uleb128-note,*note>>
                                            <| V - S - A
.2+| 62      .2+| TLSDESC_HI20      .2+| Static  | _U-Type_          .2+| High 20 bits of a 32-bit PC-relative offset into a TLS descriptor entry, `%tlsdesc_hi(symbol)`, the addend must be 0
                                            <| S - P
...
- Any other registers are callee-saved excepting the vector registers and vector CSRs when the vector extension is supported, which should be caller-saved.
...
indicates whether specific instruction encodings should be reinterpreted. For correct 
decoding by disassemblers, hints are provided to describe how Maybe instructions and 
similar encodings should be interpreted.

[horizontal]
Bit 0 (Zicfilp):: When set, `auipc x0, <value>` instructions are interpreted as `lpad <value>`.
Bit 1 (Zicfiss):: When set, Zimop/Zcmop encoding space instructions are interpreted as Zicfiss instructions.
...
set, Zimop/Zcmop encodings must be disassembled as Zicfiss instructions.
...
address of the `__jvt_base$` symbol before executing table jump instructions. It
is recommended to initialize `jvt` CSR immediately after
<<gp-relax-asm,global pointer initialization>>.

`__jvt_base$` may be undefined if the table has not been created, so the
initialization should make a weak symbol reference to avoid undefined symbol
issues.
