...
.2+| 67-76   .2+| *Reserved*                          .2+| -       |                   .2+| Reserved for Y base ISA
                                            <|
.2+| 77-79   .2+| *Reserved*                          .2+| -       |                   .2+| Reserved for future standard use
                                            <|
.2+| 80      .2+| JF                .2+| Static  | (offset field in JF) .2+| 24-bit PC-relative jump offset
                                            <| S + A - P
...
==== Function Call Relaxation (Far)
  ...
  Description:: This relaxation type can relax `AUIPC+JALR` into `JFAL`.

  Condition:: The offset between the location of relocation and target symbol or
  the PLT stub of the target symbol is within +-16MiB, and the destination register
  of the `JALR` must be `ra`.

  Relaxation::
  - Instruction sequence associated with `R_RISCV_CALL` or `R_RISCV_CALL_PLT`
  can be rewritten to a single JFAL instruction with the offset between the
  location of relocation and target symbol.
