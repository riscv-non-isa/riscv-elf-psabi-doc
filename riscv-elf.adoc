...
header fields, field not listed in this section means those field didn't have
RISC-V specific values.

e_ident::
  EI_CLASS::: Specifies the base ISA, either RV32 or RV64.  The linker disallow
  linking RV32 and RV64 code together.
+
--
[horizontal]
...
--
  EI_DATA::: Specifies the endian, either big-endian or little-endian.
  The linker disallow linking big-endian and little code together.
+
--
[horizontal]
...
--
+
NOTE: RISC-V was only support little-endian, but RISC-V spec added big-endian
since https://github.com/riscv/riscv-isa-manual/commit/2151617320bc7d27d3ce8415be306ab51c362508[Jun, 2019].
+
  EI_ABIVERSION::: Specifies the ABI version, this field have to be set to 0,
  and might defined other value by future versions of this specification.
+
...
.2+| 0    .2+| R_RISCV_NONE  .2+| None                    |                   .2+| Details
                                                         <|

.2+| 1    .2+| R_RISCV_32    .2+| Runtime relocation      | _word32_          .2+|
                                                         <| S + A
.2+| 2    .2+| R_RISCV_64    .2+| Runtime relocation      | _word64_          .2+|
                                                         <| S + A
|===
