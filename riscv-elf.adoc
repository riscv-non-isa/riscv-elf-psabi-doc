...
| Tag_RISCV_function_signature        |        7 | NTBS           | Indicates the function signature for associated symbol.
...
===== Tag_RISCV_function_signature, 7, NTBS=function signature

Tag_RISCV_function_signature is a special attribute which appled on the symbol
rather than file scope, each symbol with funciton type and external visibility
should have associate with a Tag_RISCV_function_signature if
`GNU_PROPERTY_RISCV_FEATURE_1_CFI_LP_COMPLEX` is set.
...
TODO: RISC-V has defined `Zicfilp` extension to provided...blah blah blah...

The label value for the complex labeling scheme landing pad is computed from the
hash of the function signature string, and the signature string are specified
below:
...
The `TYPE-NAME` is using the mangling name defined in <<{Cpp} Name Mangling>>,
for example, `void foo(int, int)` is encoded into `v(i,i)`.

The label value is computed from the low 20 bits of MD5 result of the signature
string, for example MD5 hash of `v(i,i)` is `3a7a68c073dcdbd6ee282a598c2dc44e`,
so the lable value is `0xdc44e`.
