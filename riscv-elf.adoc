...
WARNING: This attribute is deprecated. Due to the inability for toolchains to
distinguish between extensions that are required for execution and those that
are used in objects guarded by dynamic extension detection checks, an
object-level attribute cannot in general usefully capture an object's required
extensions, the original purpose of this attribute. For disassembly purposes,
mapping symbols provide a more accurate and fine-grained description of the
extensions present for specific regions of an object's code. Moreover, objects
could legitimately wish to perform dynamic dispatch between translation units
built with different conflicting extensions, such as `F` and `Zfinx`, which
